...
def _parse_timestamp(ts: str) -> datetime:
    """Parse an ISO-8601 timestamp string with optional fractional seconds."""
    try:
        ...
        if os.path.exists(path) and os.path.getsize(path) > 0:
            with open(path) as f:
                ...
                if line:
                    ...
                    timestamps.append(_parse_timestamp(data["timestamp"]))
    if not timestamps:
        message = (
...
def load_boxes_from_jsonl(
    ...
    def _get_frame_id(ts_str: str) -> str:
        ...
        current = _parse_timestamp(ts_str)
    ...
    with open(gt_path) as f:
        for line_number, line in enumerate(f):
            ...
            if gt_offset:
                adjusted_ts_str = (_parse_timestamp(ts) - gt_offset).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                )
            else:
                adjusted_ts_str = ts
