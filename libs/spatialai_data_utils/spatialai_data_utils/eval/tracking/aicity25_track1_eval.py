...
"""
AICity'25 Track 1 (Multi-Camera 3D People Tracking) Evaluation API.

Reproduces the per-scene, per-class HOTA evaluation protocol used by the
official AICity Challenge 2025 Track 1 validation server on top of
``spatialai_data_utils``' bundled TrackEval library so it can be re-run
locally without standing up a separate evaluation repo.

The module exposes four public callables:

* :func:`split_aicity25_per_scene_per_class` — stream-split an AICity'25
  Track 1 text file into per-(scene, class) MOT-format files (also
  useful on its own for downstream per-class analyses /
  visualizations / custom evaluators).
* :func:`run_aicity25_track1_evaluation` — end-to-end orchestrator that
...
* :func:`print_aicity25_track1_summary` — log a formatted per-(scene,
  class) and per-scene summary table to a caller-supplied logger or to
  this module's default logger.
* :func:`save_aicity25_track1_results` — persist a results dict to
  ``<output_dir>/aicity25_track1_hota_summary.json`` in the 0–100 scale
  used by the official validation server.

The CLI wrapper for this API lives at
``tools/evaluation/evaluate_aicity25_track1.py``.
...
The class-id table and the per-row field count are spec metadata, so
they live in :mod:`spatialai_data_utils.datasets.aicity25.spec` and
are imported here.  AICity'25 has exactly six classes (Person,
Forklift, NovaCarter, Transporter, FourierGR1T2, AgilityDigit at
IDs 0–5); prediction rows with ``class_id >= 6`` are not part of
the spec and are rejected as invalid.
...
# Filename written by :func:`save_aicity25_track1_results`.
_SUMMARY_JSON_NAME = "aicity25_track1_hota_summary.json"
...
def _aicity_line_to_mot(parts: List[str]) -> str:
    """Convert one AICity'25 Track 1 row into the MOT line TrackEval expects.
    ...
    zero because the AICity'25 format only carries yaw)::

        frame_id object_id 1 x y z w l h pitch roll yaw

    The ``"1"`` is TrackEval's confidence column.  We always set it to
    1.0 because the AICity'25 Track 1 text format does not carry a
...
def split_aicity25_per_scene_per_class(
    input_path: str,
    output_root: str,
    file_basename: str,
    ...
    """Stream-split an AICity'25 Track 1 file into per-(scene, class) MOT files.
    ...
    :param input_path: Path to the AICity'25 Track 1 text file (GT or
                ...
                if class_id not in valid_class_ids:
                    if is_pred:
                        raise ValueError(
                            ...
                            f"id {class_id}; valid AICity'25 Track 1 "
...
def _weighted_average(weights: Dict[str, int], values: Dict[str, float]) -> float:
    ...
    Matches the official AICity'25 Track 1 validation rule: scenes that
...
def run_aicity25_track1_evaluation(
    ...
    """End-to-end orchestrator for one (gt, pred) AICity'25 Track 1 evaluation.

    Implements the official AICity'25 Track 1 evaluation flow:
    ...
    :param ground_truth_file: Path to the AICity'25 Track 1 GT text
        file.
    :param prediction_file: Path to the AICity'25 Track 1 submission
        text file.
    :param scene_id_to_name: Mapping ``{scene_id_str: scene_name}`` for
        ...
        to get the packaged Track 1 default mapping.
    ...
    :param eval_type: ``"bbox"`` for 3D-IoU matching (the official
        Track 1 metric) or ``"location"`` for centre-distance
        matching.
    ...
    :return: A nested dict with keys ``"eval_type"``,
        ...
        as ratios in ``[0, 1]``;
        :func:`save_aicity25_track1_results` multiplies them by 100
    ...
    if output_dir is None:
        _temp_dir_handle = tempfile.TemporaryDirectory(prefix="aicity25_eval_")
    ...
    try:
        ...
        gt_counts = split_aicity25_per_scene_per_class(
            ground_truth_file, split_root, "gt.txt",
            scene_id_to_name, num_frames_to_eval, is_pred=False,
        )
        pred_counts = split_aicity25_per_scene_per_class(
...
def print_aicity25_track1_summary(results: Dict[str, Any]) -> None:
    ...
    :param results: A dict as returned by
        :func:`run_aicity25_track1_evaluation`.
    """
...
def save_aicity25_track1_results(
    results: Dict[str, Any],
    output_dir: str,
) -> str:
    """Persist evaluation *results* to ``<output_dir>/aicity25_track1_hota_summary.json``.

    The on-disk JSON multiplies every metric by 100 (matching the 0–100
    scale used by the official leaderboard and by
    :func:`print_aicity25_track1_summary`).  The in-memory *results*
    dict is **not** mutated.
