...
"""
MTMC I/O and Validation Utilities Module

This module provides input/output utilities and validation functions for MTMC
(Multi-Target Multi-Camera) tracking evaluation. It handles file path validation,
JSON data loading, BEV boundary processing, and object filtering for tracking
and detection evaluation pipelines.

Key Features:
- File path validation (existence, readability, format)
- JSON and JSON-lines file loading
- BEV boundary polygon parsing and validation
- Object classification and sub-class mapping
- Confidence-based filtering
- Custom argparse actions for file validation

Main Components:
- ValidateFile: Custom argparse action for file path validation
- validate_file_path: Validate file path existence and format
- load_json_from_file: Load JSON data from files
- load_json_lines_from_file: Load JSON-lines format files
- parse_bev_boundary: Parse BEV boundary polygon from strings
- filter_objects_by_confidence: Filter detections/tracks by confidence
- map_object_class: Map sub-classes to primary classes

File Validation:
- Check file existence and readability
- Validate path format (alphanumeric, dash, underscore, slash)
- Integration with argparse for CLI tools

BEV Boundary Format:
- String representation: "[[x1,y1], [x2,y2], [x3,y3], [x4,y4]]"
- Parsed to list of coordinate pairs
- Used for spatial filtering of detections

Object Filtering:
- Confidence threshold filtering
- Sub-class to primary class mapping
- Support for various confidence field names

Use Cases:
- Validate input files in evaluation scripts
- Load tracking and detection results
- Parse BEV boundaries from configuration
- Filter low-confidence detections
- Standardize object class names

Typical Workflow:
1. Validate file paths for tracking/detection results
2. Load JSON data with error handling
3. Parse BEV boundaries for spatial filtering
4. Filter objects by confidence threshold
5. Map sub-classes to primary classes
6. Use cleaned data for evaluation
"""
