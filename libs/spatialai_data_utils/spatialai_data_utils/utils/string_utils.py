...
"""
Generic numeric-string helpers (regex digit extraction + natural sort key).

This module hosts the **language-level** primitives that downstream
camera-name handling code is built on. They contain no camera /
scene / dataset knowledge and can be applied to any string.
...
* :func:`natural_sort_key` — the ``key=`` callable backing natural
  sort. Splits a string into alphabetic and numeric chunks for
  correct ``"item2" < "item10"`` ordering.
* :func:`extract_numbers` — extract every digit run from a string
  as a list of ints.
"""
